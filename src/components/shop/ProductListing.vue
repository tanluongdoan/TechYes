<template>
  <div :class="productWrapClass" class="pb-32">
    <aside class="sidebar">
      <div>
        <PriceFilter :productCustomFilters="productCustomFilters" />
      </div>
      <div v-for="(value, key) in props.productFilters" :key="key">
        <ProductFilter :model-value="productCustomFilters" :value="value" :name="key" />
      </div>
    ...
    <div class="products-container">
      <ul :class="`${productWrapClass} pb-20`">
        <li v-for="item in productListing" :key="item.ProductCode" class="custom-grid-item overflow-hidden">
          <div class="card-wrapper">
            <a href="/" class="flex-card fade-up group relative z-10 flex max-w-md flex-1 flex-col items-center gap-4 rounded-md text-center">
              <div class="h-40 w-40 shrink-0 grow-0 overflow-hidden">
                <div class="transition-transform duration-500 group-hover:scale-110 group-hover:duration-[3s]">
                  <img class="block" :src="item.FeaturedImage" :alt="item.FeaturedImage" />
                </div>
              </div>
      ...
      <div v-if="productListing" class="flex items-center justify-end space-x-1 py-2 pt-12">
        <!-- prev button (previous page) -->
        <button @click="gotoPage(currentPage - 1)" :disabled="currentPage === 1" class="flex h-8 w-8 items-center justify-center rounded hover:border-black hover:bg-primary hover:text-white disabled:pointer-events-none">
        ...
        <div v-for="page in sizePage" :key="page" class="flex items-center justify-between">
          <button @click="gotoPage(page)" :disabled="page === currentPage" class="block h-8 w-8 rounded border-none border-gray-100 text-center leading-8 hover:bg-primary hover:text-white disabled:bg-primary disabled:text-white">
            {{ page }}
          </button>
        </div>

        <!-- next button (next page) -->
        <button @click="gotoPage(currentPage + 1)" :disabled="currentPage === sizePage" class="flex h-8 w-8 items-center justify-center rounded hover:border-black hover:bg-primary hover:text-white disabled:pointer-events-none">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
